...
## 更新摘要
**所做更改**
- 新增发布失败状态筛选功能，支持publish_status=failed的过滤
- 改进URL参数处理，优化草稿管理界面的筛选逻辑
- 增强空状态显示，提供更友好的用户体验
- 在Dashboard中增加发布失败状态的展示和链接
- 完善发布历史记录的失败状态标识和样式
...
- **发布失败状态筛选**：新增发布失败状态的专门筛选功能，支持publish_status=failed的过滤
...
DraftCheck --> DraftStatus[草稿状态检查]
DraftCheck --> Idempotency[幂等性检查]
...
**更新** 草稿管理界面新增发布失败状态筛选功能

草稿管理界面提供了完整的草稿生命周期管理功能，支持查看、操作和重跑草稿。新增了专门的发布失败状态筛选功能，支持通过URL参数publish_status=failed进行过滤。
...
**URL参数处理改进**：
- 支持publish_status=failed参数进行筛选
- 改进resolveTabByQuery函数，正确处理发布失败状态
- 优化草稿状态徽章显示，针对失败状态使用红色样式

**空状态显示增强**：
- 为发布失败状态提供专门的空状态图标和文案
- 增强的引导动作，帮助用户处理失败的草稿
- 统一的视觉设计，与其他状态保持一致

**章节来源**
- [frontend/app/drafts/page.tsx:11-472](file://frontend/app/drafts/page.tsx#L11-L472)

### 运营总览界面

**更新** Dashboard中增加发布失败状态的展示

运营总览界面展示了系统的整体运行状况，新增了发布失败状态的统计和链接功能。

```mermaid
flowchart TD
Dashboard[运营总览] --> Stats[核心运营指标]
Stats --> TodayTasks[今日任务]
Stats --> PendingDrafts[待确认]
Stats --> PublishedToday[今日发布]
Stats --> PublishFailed[发布失败]
PublishFailed --> FailedLink[点击跳转到失败草稿筛选]
```

**Dashboard功能**：
- 展示四大核心运营指标：今日任务、待确认、今日发布、发布失败
- 发布失败指标使用红色主题，突出显示问题状态
- 点击发布失败指标可直接跳转到草稿管理界面的失败状态筛选
- 实时统计数据更新，每30秒刷新一次

**章节来源**
- [frontend/app/(shell)/layout.tsx:58-64](file://frontend/app/(shell)/layout.tsx#L58-L64)
- [frontend/app/(shell)/page.tsx:214-215](file://frontend/app/(shell)/page.tsx#L214-L215)

### 草稿详情界面

**更新** 草稿详情界面增强发布历史记录的失败状态标识

草稿详情界面提供了草稿的完整信息和操作功能，增强了发布历史记录的失败状态显示。

```mermaid
flowchart TD
DraftDetail[草稿详情] --> PublishStatus[微信发布状态]
PublishStatus --> StatusBadge[状态徽章]
StatusBadge --> Published[已发布]
StatusBadge --> Failed[发布失败]
StatusBadge --> Pending[发布中]
Failed --> ErrorMessage[失败原因显示]
PublishHistory[发布历史] --> HistoryItems[历史记录项]
HistoryItems --> SuccessItem[成功记录绿色]
HistoryItems --> FailedItem[失败记录红色]
HistoryItems --> PendingItem[进行中记录黄色]
```
...
**章节来源**
- [frontend/app/drafts/[id]/page.tsx:373-414](file://frontend/app/drafts/[id]/page.tsx#L373-L414)
...
**更新** 前端API客户端支持发布失败状态筛选

前端API客户端封装了所有后端API调用，提供了统一的接口和错误处理机制。新增了对发布失败状态筛选的支持。
...
**章节来源**
- [frontend/lib/api.ts:53-729](file://frontend/lib/api.ts#L53-L729)
...
- **新增** 发布失败状态筛选功能，支持publish_status=failed的过滤
...
- **更新** 发布失败状态的专门处理机制，提供更好的用户体验
- **新增** 发布失败状态的专门筛选功能，支持publish_status=failed的过滤
...
**新增** 草稿管理界面增强价值：
- **新增** 发布失败状态筛选功能，支持publish_status=failed的过滤
- **新增** 改进的URL参数处理，优化筛选逻辑
- **新增** 增强的空状态显示，提供更友好的用户体验
- **新增** 发布历史记录的失败状态标识和样式
- **新增** 发布失败状态的专门徽章和颜色标识
...
该系统为公众号内容生产提供了一套完整的解决方案，既满足了技术先进性的要求，又保证了良好的用户体验和系统的可维护性。通过模块化的设计和灵活的配置机制，系统能够适应不同的业务需求和应用场景。新增的微信发布功能、发布记录服务系统、发布决策服务和发布失败状态筛选功能进一步增强了系统的实用性和可靠性，为用户提供从内容创作到实际发布的完整解决方案。更新后的发布功能通过重试机制和状态同步，显著提高了发布的成功率和系统的稳定性。发布保护层的引入和发布失败状态的专门处理则确保了发布过程的安全性和合规性，为内容生产的质量控制提供了强有力的保障。