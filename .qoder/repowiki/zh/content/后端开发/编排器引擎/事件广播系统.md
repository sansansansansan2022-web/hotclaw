...
- [backend/tests/test_broadcaster.py](file://backend/tests/test_broadcaster.py)
</cite>
...
- 关键能力
  ...
  - **容量限制**：每个任务最多保留固定数量的历史事件，防止内存无限增长
- 数据结构
  - 订阅者映射：task_id → 队列列表
  - 历史缓冲：task_id → 事件列表（具有容量限制）
  - 关闭标记：task_id → 布尔值

**更新** 新增历史事件容量限制功能，每个任务默认最多保留200个事件

```mermaid
classDiagram
class SSEBroadcaster {
-_max_history_per_task : int
...
**图表来源**
- [backend/app/orchestrator/broadcaster.py:49-56](file://backend/app/orchestrator/broadcaster.py#L49-L56)
- [backend/app/orchestrator/broadcaster.py:111-145](file://backend/app/orchestrator/broadcaster.py#L111-L145)

**章节来源**
- [backend/app/orchestrator/broadcaster.py:30-94](file://backend/app/orchestrator/broadcaster.py#L30-L94)
- [backend/app/orchestrator/broadcaster.py:129-136](file://backend/app/orchestrator/broadcaster.py#L129-L136)
...
## 性能考虑
- 内存优化
  - **历史缓冲容量限制**：广播器为每个任务维护历史事件列表，用于回放；通过容量限制（默认200个事件）防止长时间运行任务导致的内存无限增长
  - 订阅者队列：使用 asyncio.Queue，具备背压与异步特性，适合高并发场景
  - **智能淘汰机制**：当历史事件超过容量限制时，自动移除最早的历史事件，保持固定上限
- 超时与保活
...
**更新** 新增历史事件容量限制功能，解决长时间运行任务的内存增长问题

**章节来源**
- [backend/app/orchestrator/broadcaster.py:49-50](file://backend/app/orchestrator/broadcaster.py#L49-L50)
- [backend/app/orchestrator/broadcaster.py:129-136](file://backend/app/orchestrator/broadcaster.py#L129-L136)
...
- 常见问题
  ...
  - **历史事件截断**：如果发现某些历史事件缺失，这是由于容量限制导致的正常现象
- 日志与追踪
...
## 结论
事件广播系统通过"编排引擎/服务层 → 广播器 → SSE 路由 → 前端 Hook"的链路，实现了低延迟、可回放的任务执行事件流。系统在连接保活、历史回放、异常广播与资源清理方面具备完整设计，配合结构化日志与前端自动重连机制，能够满足生产环境的稳定性与可观测性需求。**新增的历史事件容量限制功能有效解决了长时间运行任务的内存增长问题，确保系统在高负载场景下的稳定运行。**
...
- **容量限制**：每个任务的历史事件缓冲区具有固定容量上限（默认200个事件），超出部分自动淘汰
...
### 历史事件容量限制配置
- **默认容量**：每个任务默认保留200个历史事件
- **淘汰机制**：当事件数量超过容量限制时，自动移除最早的历史事件
- **测试验证**：通过单元测试验证容量限制功能，确保只保留最新的200个事件
- **内存保护**：有效防止长时间运行任务导致的内存无限增长
...
- [backend/app/orchestrator/broadcaster.py:49-50](file://backend/app/orchestrator/broadcaster.py#L49-L50)
- [backend/app/orchestrator/broadcaster.py:129-136](file://backend/app/orchestrator/broadcaster.py#L129-L136)
- [backend/tests/test_broadcaster.py:9-20](file://backend/tests/test_broadcaster.py#L9-L20)