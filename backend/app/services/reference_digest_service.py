...
class ReferenceDigestService:
    ...
    def _order_sources(self, sources: list[dict[str, Any]], preferred_ids: list[str]) -> list[dict[str, Any]]:
    ...
    def _usage_rules(self, source_digests: list[dict[str, Any]]) -> list[str]:
        if not source_digests:
            return ["No preferred reference source is available; rely on account positioning and avoid inventing citations."]
        return [
