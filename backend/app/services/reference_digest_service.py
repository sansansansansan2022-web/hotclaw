"""Reference source digest helpers for article assembly prompts."""
...
class ReferenceDigestService:
    """Build compact, deterministic reference-source summaries."""
    ...
    def build_reference_digest(
        ...
        *,
        ...
        preferred_ids = self._preferred_reference_ids(ops_context)
        ...
        return {
            ...
            "style_takeaways": self._style_takeaways(selected_sources),
            "structure_takeaways": self._structure_takeaways(selected_sources),
            "usage_rules": self._usage_rules(selected_sources),
    ...
    def _preferred_reference_ids(self, ops_context: dict[str, Any]) -> list[str]:
        ...
        if not isinstance(run_strategy, dict):
            return []
        preferred_ids = run_strategy.get("preferred_reference_source_ids")
        if not isinstance(preferred_ids, list):
            return []
        normalized: list[str] = []
        for item in preferred_ids:
    ...
    def _prioritize_reference_sources(
        ...
        *,
        ...
        return sorted(sources, key=_sort_key)[: max(limit, 0)]

    def _style_takeaways(self, sources: list[dict[str, Any]]) -> list[str]:
        takeaways: list[str] = []
        for source in sources:
            clue = source.get("style_clues") or source.get("notes") or source.get("preview")
            text = self._clip_text(clue, 140)
            if text:
                takeaways.append(f"{source['name']}: {text}")
        return takeaways[:6]

    def _structure_takeaways(self, sources: list[dict[str, Any]]) -> list[str]:
        takeaways: list[str] = []
        for source in sources:
            preview = self._clip_text(source.get("preview"), 140)
            if preview:
                takeaways.append(f"Use {source['name']} as a structural reference: {preview}")
        return takeaways[:6]

    def _usage_rules(self, sources: list[dict[str, Any]]) -> list[str]:
        if not sources:
            return []
        names = ", ".join(source["name"] for source in sources if source.get("name"))
        return [
            f"Borrow pacing and structure from selected references ({names}) without copying phrasing.",
            "Keep the generated article aligned with the selected topic and account positioning.",
        ]
