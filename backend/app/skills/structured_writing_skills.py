"""Shared structured writing skills and compatibility mixins."""
...
"""
    ...
    async def generate_outline(
        ...
        *,
        ...
        if isinstance(outline_seed, dict) and outline_seed.get("sections"):
            return self._normalize_outline(outline_seed, input_data)
        ...
        return self._normalize_outline(self._parse_json(content), input_data)
    ...
    def _build_user_prompt(self, input_data: dict[str, Any]) -> str:
        ...
        ops_snapshot = {
            ...
            "preferred_reference_source_ids": (ops_context.get("run_strategy") or {}).get(
        ...
        return "\n".join(
            [
                ...
                article_assembler_service.to_pretty_json(
                    source_candidates[:5] if isinstance(source_candidates, list) else []
                ),
                "",
                "RETURN CONTRACT",
    ...
    def _outline_matches_topic(self, outline: dict[str, Any], selected_topic: str, selected_title: str) -> bool:
        combined_parts = [
    ...
    def _outline_fallback_payload(self, input_data: dict[str, Any]) -> dict[str, Any]:
        selected_title = article_assembler_service.extract_selected_title(input_data.get("titles"))
        selected_topic = article_assembler_service.extract_selected_topic(
            input_data.get("topics"), input_data.get("titles")
        )
        ...
        sections = [
            ...
            self._build_section(
                ...
                [
                    ...
                    useful_points[1]
                    if len(useful_points) > 1
                    else "Let this section feel like the article's turning point, not just another parallel point.",
                ],
                "Sharper, still human",
...
class SectionDraftMixin:
    ...
    output_schema = {"type": "object", "properties": {"section_drafts": {"type": "array"}}}
...
"""
    ...
    async def generate_section_drafts(self, *, input_data: dict[str, Any], context: dict[str, Any], completion_callable) -> dict[str, Any]:
        system_prompt = self.get_system_prompt(context)
        user_prompt = self._build_user_prompt(input_data)
        node_timeout = context.get("node_timeout_seconds")
        try:
        ...
        content = response.choices[0].message.content
        return self._normalize_section_drafts(self._parse_json(content))
    ...
    def _build_user_prompt(self, input_data: dict[str, Any]) -> str:
        ...
        return "\n".join(
            [
                ...
                f"Stay locked to this exact topic/title pair: {selected_topic} / {selected_title}.",
                "",
                "ACCOUNT SNAPSHOT",
                article_assembler_service.to_pretty_json(account_snapshot),
                "",
    ...
    def _section_drafts_match_topic(self, section_drafts: dict[str, Any], selected_topic: str, selected_title: str) -> bool:
        drafts = section_drafts.get("section_drafts") if isinstance(section_drafts, dict) else []
        if not isinstance(drafts, list):
            return False
        combined_text = " ".join(
            " ".join([str(item.get("heading") or ""), str(item.get("summary") or ""), str(item.get("content_markdown") or "")])
            for item in drafts
            if isinstance(item, dict)
        )
        return article_assembler_service.text_matches_topic(combined_text, selected_topic=selected_topic, selected_title=selected_title)
    ...
    def _section_fallback_payload(self, error: Exception, input_data: dict[str, Any]) -> dict[str, Any]:
        outline = input_data.get("outline_plan") or {}
        title = article_assembler_service.extract_selected_title(input_data.get("titles"))
        topic = article_assembler_service.extract_selected_topic(input_data.get("topics"), input_data.get("titles"))
        ...
        for index, section in enumerate(outline.get("sections", [])):
            ...
            transition_hint = str(section.get("section_transition_hint") or section.get("transition_hint") or "").strip()
            paragraphs = [f"In '{title}', this section turns toward {heading.lower()} with a clear job to do."]
            if topic:
...
class OutlineGenerateSkill(_SkillRuntimeMixin, BaseSkill, OutlineGenerateMixin):
    skill_id = "outline_generate_skill"
    name = "Outline Generate Skill"
    description = "Generate a structured outline package from selected topic, title, and reference context."

    async def execute(self, input_data: dict) -> dict:
        context = input_data.get("_context") if isinstance(input_data.get("_context"), dict) else {}
        try:
            data = await self.generate_outline(input_data=input_data, context=context, completion_callable=litellm.acompletion)
...
class SectionDraftSkill(_SkillRuntimeMixin, BaseSkill, SectionDraftMixin, OutlineGenerateMixin):
    skill_id = "section_draft_skill"
    name = "Section Draft Skill"
    description = "Write section-level drafts from a structured outline package."

    async def execute(self, input_data: dict) -> dict:
        context = input_data.get("_context") if isinstance(input_data.get("_context"), dict) else {}
        try:
            data = await self.generate_section_drafts(input_data=input_data, context=context, completion_callable=litellm.acompletion)
            return SkillResult.success(self.skill_id, data).to_dict()
        except Exception as exc:
            return SkillResult.failure(self.skill_id, "LLM_ERROR", str(exc)).to_dict()


outline_generate_skill = OutlineGenerateSkill()
section_draft_skill = SectionDraftSkill()
